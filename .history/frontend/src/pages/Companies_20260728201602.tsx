...
import { getCompanies, createCompany, updateCompany } from "../services/company.service";
// Note: deleteCompany not exported from service - add it or remove delete functionality
import Button from "../components/Button";
import CompanyModal, { CompanyData, Owner } from "../components/CompanyModal";

// TODO: Replace with actual API call when ready
// import { getOwners } from "../services/owner.service";

const mockOwners: Owner[] = [
  { _id: "1", name: "Mahmud Hasan Shanto" },
  { _id: "2", name: "John Doe" },
];
...
const Companies = () => {
  ...
  useEffect(() => {
    fetchCompanies();
    // fetchOwners(); // Uncomment when owner service is ready
  }, []);
  ...
  const handleOpenEditModal = (company: CompanyData) => {
  ...
  // TODO: Add deleteCompany to your service file or remove this function
  const handleDeleteCompany = async (companyId: string) => {
    if (window.confirm("Are you sure you want to delete this company?")) {
      try {
        // Uncomment when deleteCompany is available in your service
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <Button onClick={handleOpenAddModal}>
            <Plus className="w-5 h-5 mr-1" /> Add Company
          </Button>
        </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-xl shadow-violet-500/10 border border-slate-200 dark:border-slate-700">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 dark:text-white mt-2">
                  {companies.filter((c) => c.owner).length}
                </p>
              </div>
                ...
                <div className="p-6 pt-4">
                    ...
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-slate-900 dark:text-white">
                          {typeof company.owner === "object" ? company.owner?.name || "No Owner" : "No Owner"}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-2 pt-4 border-t border-slate-200 dark:border-slate-700">
                    <button 
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-xl font-medium hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-all"
                    >
                    ...
                    <button 
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-xl font-medium hover:bg-amber-100 dark:hover:bg-amber-900/50 transition-all"
                    >
                    ...
                    <button 
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-xl font-medium hover:bg-red-100 dark:hover:bg-red-900/50 transition-all"
                    >
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-12 shadow-xl shadow-slate-500/10 border border-slate-200 dark:border-slate-700 text-center">
            ...
            {!search && (
              <button 
                ...
                className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-medium hover:shadow-lg hover:shadow-blue-500/50 transition-all duration-300 inline-flex items-center gap-2"
              >
